...
class LayerInfo(NamedTuple):
    ...
    def from_string(cls, string, tactic_names, layer_times=None):
        ...
        time = layer_times[layer_name] if layer_name in layer_times.keys() else "NA"
        return cls(
            kernel_name,
            layer_name,
            tactic,
...
if args.profile_file != "":
    ...
    for t in times:
        ...
        res = re.findall(
            ".*\"name\" : \"(.*)\", \"timeMs.* \"averageMs\" : (.*), \"percentage.*", t)
        if len(res) > 0:
            name, time = res[0]
            layer_times[name] = time
...
if args.log_file != "":
    ...
    for line in lines:
        ...
        if tactic_name_start and "Set Tactic Name:" in line:
            res = re.findall(
                "\[V\] \[TRT\] (.*) Set Tactic Name: (.*) Tactic: (.*)$", line)
            layer_name, kernel_name, _ = res[0]
    ...
    for layers, reformat_layers in graphs:
        ...
        dot_graph.write_png(f"EngineLayers_{i}.png")
        i += 1
...
if args.profile_file != "":
    ...
    for layer in layers:
        time = float(layer.time[:-2]) if layer.time != 'NA' else 0
        if layer.kernel_name == "Reformat":
            est_reformat_time += time
        est_total_time += time
